...
sf.1A4ai <- st_read("D:/R_projects/Spatialization/2030_WAM_A/Products_2030_WAM_A/1A4 - Residential-Tertiary_2030_WAM_A/1A4ai.gpkg")
...
sf.1A4ai.tl <- lapply(sf.1A4ai.tl, function(x) data.frame(x) %>% mutate(Time = activity.df$times_2030_WAM_A) %>% dplyr::select(Time, everything())%>% dplyr::rename_at(vars(-1), ~ paste(ids)))
...
#+ include = FALSE
sf.1A4bi <- st_read("D:/R_projects/Spatialization/2030_WAM_A/Products_2030_WAM_A/1A4 - Residential-Tertiary_2030_WAM_A/1A4bi.gpkg")
...
rush_hour0509 <- day_hours %in% c(6:10)
activity.df$rush_hour0709 <- rush_hour0509

activity.df %<>% 
  dplyr::mutate(RH0509 = dplyr::case_when(rush_hour0509 == TRUE ~ 0.5*sin(((2*pi)/24)*(day_hours-3)) + 0.5,
                                          rush_hour0509 == FALSE ~ 0))


rush_hour1821 <- day_hours %in% c(19:22)
activity.df$rush_hour1821 <- rush_hour1821

activity.df %<>% 
  dplyr::mutate(RH1821 = dplyr::case_when(rush_hour1821 == TRUE ~ 0.5*sin(((2*pi)/24)*(day_hours-11)) + 0.5,
                                          rush_hour1821 == FALSE ~ 0))

activity.df %<>%
  dplyr::mutate(TEMP5 = dplyr::case_when(TEMP < 5 ~ TEMP,
                                         TEMP > 5 ~ 0,
                                         TEMP == 5 ~ 5))
...
sf.1A4bi.tl <- lapply(sf.1A4bi.tl, function(x) data.frame(x) %>% mutate(Time = activity.df$times_2030_WAM_A) %>% dplyr::select(Time, everything())%>% dplyr::rename_at(vars(-1), ~ paste(ids)))
...
#+ include = FALSE
sf.1A4ci <- st_read("D:/R_projects/Spatialization/2030_WAM_A/Products_2030_WAM_A/1A4 - Residential-Tertiary_2030_WAM_A/1A4ci.gpkg")
...
sf.1A4ci.tl <- lapply(sf.1A4ci.tl, function(x) data.frame(x) %>% mutate(Time = activity.df$times_2030_WAM_A) %>% dplyr::select(Time, everything())%>% dplyr::rename_at(vars(-1), ~ paste(ids)))
...
sf.1A4cii <- st_read("D:/R_projects/Spatialization/2030_WAM_A/Products_2030_WAM_A/1A4 - Residential-Tertiary_2030_WAM_A/1A4cii.gpkg")
...
he.1A4cii <- activity.df %>%
  ...
  dplyr::mutate(SAAG1 = dplyr::case_when(WE == TRUE ~ 1,
                                         WE == FALSE ~ 0)) %>%
  dplyr::mutate(SAAG2 = (sin(((2*pi)/12)*(!SAAG1))+0.5)) %>%
  dplyr::mutate(he_1A4cii = SAAG.f + SAAG.fl) %>%
...
sf.1A4cii.tl <- lapply(sf.1A4cii.tl, function(x) data.frame(x) %>% mutate(Time = activity.df$times_2030_WAM_A) %>% dplyr::select(Time, everything())%>% dplyr::rename_at(vars(-1), ~ paste(ids)))
...
temporalProfile_ResidentialTertiary <- activity.df$times_2030_WAM_A %>% 
  cbind(he.1A4ai[,1:6], 
        he.1A4bi[,1:6], 
        he.1A4ci[,1:6], 
        he.1A4cii[,1:6]) %>% 
...
writexl::write_xlsx(temporalProfile_ResidentialTertiary, path = '2030_WAM_A/Hourly_emissions_2030_WAM_A/Products_2030_WAM_A/TemporalProfile_Residential_Tertiary_2030_WAM_A.xlsx')
